...
final class FueTrekSampler implements Sampler {
    ...
    private static final int MIX_PROFILE_MODE = 0;
    private static final int EVENT_X_DRUM_ENABLE = 0xBA;
    private static final int EVENT_MASTER_VOLUME = 0xB0;
    ...
    private static final int EVENT_PITCHBEND_RANGE = 0xE7;
    private static final int EVENT_WAVE_CHANNEL_VOLUME = 0xE8;
    private static final int EVENT_WAVE_CHANNEL_PANPOT = 0xE9;
    ...
    private final int[] legacyYamahaSelectorProgramSlots = new int[LEGACY_YAMAHA_SLOT_COUNT];
    ...
    FueTrekSampler(FueTrekRom rom, float sampleRate, int maxPolyphony) {
        ...
        Arrays.fill(legacyYamahaSelectorProgramSlots, -1);
    }

    private static boolean debugNotesEnabled() {
        return SmafDebug.isEnabled("smaf", SmafDebug.Level.DEBUG);
    }

    private static boolean debugControlEnabled() {
        return SmafDebug.isEnabled("smaf", SmafDebug.Level.DEBUG);
    }
    ...
    public void keyOn(int channel, int key, float velocity) {
        ...
        Voice voice = new Voice(
                ...
                MIX_PROFILE_MODE,
                nextVoiceAge++);
        if (debugNotesEnabled()) {
            SmafDebug.debug("smaf", String.format(
                    "[FueTrekNote] ch=%d key=%d noteByte=%d group=0x%02x sub=%d sel=%d rootA=%d rootB=%d tuneA=%d tuneB=%d baseNote=%d staticQ16=%d oscBMode=%d auxLoaded=%s auxNote=%d auxPitch=%d rawPitch=0x%04x bendRange=%d",
                    channel,
                    key,
                    resolved.noteByte,
        ...
        int slotIndex = allocateVoiceSlot(voice);
        if (slotIndex < 0) {
            return;
        }
    ...
    public void sysEx(int sourceBank, byte[] message) {
        ...
        if (looksLikeUnsupportedRawYamahaPacket(message)) {
            debugUnsupportedRawYamahaPacket(message);
            return;
        }
    ...
    private boolean applyRawLegacyYamahaVoice(SoftbankExvoPacket packet, byte[] message) {
        ...
        RawExvoProgramSlot slot = new RawExvoProgramSlot(
                ...
                program,
                null,
                decodeLegacyYamahaSelectorBankByte(message, packet.payloadOffset()),
                decodeLegacyYamahaSelectorProgramByte(message, packet.payloadOffset()));
        rawExvoProgramSlots[programSlot] = slot;
        loadedLegacyVmProgramCount = Math.max(loadedLegacyVmProgramCount, programSlot + 1);
        rememberLegacyYamahaSelectorProgram(programSlot);
    ...
    private boolean applyRawLegacyYamahaNoteControl(SoftbankExvoPacket packet, byte[] message, int sourceBank) {
        ...
        configureLegacyYamahaHiddenChannel(slot, keyOn ? noteWord : slot.activeNoteWord);
    ...
    private boolean applyInternalLegacyYamahaMessage(int sourceBank, byte[] message) {
        ...
        switch (message[1] & 0xff) {
            ...
            case INTERNAL_LEGACY_YAMAHA_MODULATION:
                slot.modulationByte = value & 0x7f;
                refreshLegacyYamahaActiveChannel(slot);
                return true;
            case INTERNAL_LEGACY_YAMAHA_VOLUME:
                slot.volumeByte = value & 0x7f;
                refreshLegacyYamahaActiveChannel(slot);
                return true;
            case INTERNAL_LEGACY_YAMAHA_PAN:
                slot.panByte = value & 0x7f;
                refreshLegacyYamahaActiveChannel(slot);
                return true;
            case INTERNAL_LEGACY_YAMAHA_PHRASE_VOLUME:
                slot.phraseVolumeByte = value & 0x7f;
                refreshLegacyYamahaActiveChannel(slot);
                return true;
            case INTERNAL_LEGACY_YAMAHA_PROGRAM:
                slot.selectorProgramByte = value & 0x7f;
                refreshLegacyYamahaActiveChannel(slot);
                return true;
            case INTERNAL_LEGACY_YAMAHA_BANK:
                slot.selectorBankByte = value & 0xff;
                refreshLegacyYamahaActiveChannel(slot);
                return true;
            case INTERNAL_LEGACY_YAMAHA_PITCH:
                slot.pitchWord = clampRange((value & 0x7f) << 7, 0, 0x3fff);
                refreshLegacyYamahaActiveChannel(slot);
                return true;
            case INTERNAL_LEGACY_YAMAHA_BEND_RANGE:
                slot.bendRangeByte = clampRange(value & 0x7f, 0, 0x18);
                refreshLegacyYamahaActiveChannel(slot);
                return true;
            default:
                return false;
    ...
    private void configureLegacyYamahaHiddenChannel(LegacyYamahaSlotState slot, int noteWord) {
        ...
        state.modPair19Byte = slot.modulationByte;
        state.setRawPitchWord(legacyYamahaPitchWord(slot, noteWord));
        state.setBendRangeByte(slot.bendRangeByte);
        applyLegacyYamahaBaseObject(state, slot, noteWord);
    ...
    private void refreshLegacyYamahaActiveChannel(LegacyYamahaSlotState slot) {
        ...
        state.expressionByte = slot.expressionByte;
        state.modPair19Byte = slot.modulationByte;
        state.setRawPitchWord(legacyYamahaPitchWord(slot, slot.activeNoteWord));
        state.setBendRangeByte(slot.bendRangeByte);
        volume(slot.hiddenChannel(), slot.channelVolume());
        panpot(slot.hiddenChannel(), slot.panpot());
        updateActiveVoiceModSeed(state);
    ...
    private void applyPendingLegacyYamahaProgram(LegacyYamahaSlotState slot) {
        ...
        int selectorIndex = clampRange(slot.pendingProgramSlot, 0, LEGACY_YAMAHA_SLOT_COUNT - 1);
        int mappedProgram = legacyYamahaSelectorProgramSlots[selectorIndex];
        if (mappedProgram >= 0) {
            slot.programSlot = clampRange(mappedProgram, 0, 0x3f);
        } else if (selectorIndex < loadedLegacyVmProgramCount) {
            slot.programSlot = selectorIndex;
        }
        slot.pendingProgramSlot = -1;
    }

    private void rememberLegacyYamahaSelectorProgram(int programSlot) {
        if (programSlot < 0 || programSlot >= 0x40) {
            return;
        }
        int selectorIndex = clampRange(programSlot, 0, LEGACY_YAMAHA_SLOT_COUNT - 1);
        if (legacyYamahaSelectorProgramSlots[selectorIndex] < 0) {
            legacyYamahaSelectorProgramSlots[selectorIndex] = programSlot;
        }
    }

    private static int decodeLegacyYamahaExtendedKey(int noteWord) {
        int midiNote = clampRange(noteWord >> 7, 0, 0x7f);
        return midiNote - MIDI_A4;
    }

    private static int legacyYamahaPitchWord(LegacyYamahaSlotState slot, int noteWord) {
        int basePitchWord = clampRange(slot.pitchWord, 0, 0x3fff);
        if (noteWord < 0 || slot.bendRangeByte <= 0) {
            return basePitchWord;
        }
        int noteFraction = noteWord & 0x7f;
        if (noteFraction == 0) {
            return basePitchWord;
        }
        int fineOffset = Math.round((noteFraction * 64.0f) / slot.bendRangeByte);
        return clampRange(basePitchWord + fineOffset, 0, 0x3fff);
    }

    private void applyLegacyYamahaBaseObject(ChannelState state, LegacyYamahaSlotState slot, int noteWord) {
        if (state == null) {
            return;
        }
        RawExvoProgramSlot programSlot = rawExvoProgramSlots[slot.programSlot & 0x3f];
        if (programSlot == null || programSlot.groupId >= 0 || noteWord < 0) {
            state.clearLegacyYamahaBaseObject();
            return;
        }
        if (slot.selectorProgramByte >= 0 && slot.selectorBankByte >= 0) {
            state.clearLegacyYamahaBaseObject();
            state.bank = normalizeLegacyYamahaSelectorBank(slot.selectorBankByte);
            state.program = slot.selectorProgramByte & 0x7f;
            state.programSelected = true;
            return;
        }
        if (programSlot.legacyBankByte >= 0 && programSlot.legacyProgramByte >= 0) {
            state.clearLegacyYamahaBaseObject();
            state.bank = normalizeLegacyYamahaSelectorBank(programSlot.legacyBankByte);
            state.program = programSlot.legacyProgramByte & 0x7f;
            state.programSelected = true;
            return;
        }
        int noteByte = clampRange(noteWord >> 7, 0, 0x7f);
        if (noteByte >= 0x23 && noteByte <= 0x42 && state.applyRawObjectOverride(rom, 0x14, noteByte)) {
            return;
        }
        state.clearLegacyYamahaBaseObject();
    }

    private static int normalizeLegacyYamahaSelectorBank(int rawBankByte) {
        int lowBank = rawBankByte & 0x7f;
        int familyBit = (rawBankByte & 0x80) != 0 ? 1 : 0;
        return clampRange((lowBank << 1) | familyBit, 0, 0x7f);
    }
    ...
    private static int decodeLegacyYamahaSelectorBankByte(byte[] message, int offset) {
        int index = offset + 4;
        if (message == null || index < 0 || index >= message.length) {
            return -1;
        }
        return message[index] & 0x7f;
    }

    private static int decodeLegacyYamahaSelectorProgramByte(byte[] message, int offset) {
        int index = offset + 5;
        if (message == null || index < 0 || index >= message.length) {
            return -1;
        }
    ...
    private static DecodedCompactValue decodeRawExvoCompactValue(byte[] message, int offset) {
        ...
        int first = message[offset] & 0xff;
        if (offset == message.length - 1 && first == 0xf7) {
            return null;
        }
        if (first <= 0xdf || offset + 1 >= message.length) {
            return new DecodedCompactValue(first, offset + 1);
        }
        int second = message[offset + 1] & 0xff;
        int value = ((first & 0x7) << 8) | second;
        if ((first & 0x8) != 0) {
            value |= ~0x7ff;
        }
        return new DecodedCompactValue(value, offset + 2);
    }

    private static RawExvoUpload81 parseRawExvoUpload81(byte[] message, int offset) {
        if (offset >= message.length) {
            return null;
        }
        int end = message.length;
    ...
    private static void debugRawExvoOverride(
        ...
        sb.append("[FueTrekCtrl] raw-exvo ")
                .append(source)
                .append(" ch=")
                .append(channelIndex)
                .append(" group=0x")
                .append(String.format("%02x", groupId & 0xff))
                .append(" obj=0x")
                .append(String.format("%02x", objectIndex & 0xff))
                .append(" data=");
    ...
    private static void debugRawExvoUpload81(RawExvoUpload81 upload, byte[] message) {
        if (!debugControlEnabled()) {
            return;
        }
        StringBuilder sb = new StringBuilder(192);
        sb.append("[FueTrekCtrl] raw-exvo upload81 len=")
                .append(upload.payload.length)
                .append(" pair0=")
    ...
    private static byte[] unwrapRawVendorEnvelope(byte[] message) {
        ...
        int end = (message[message.length - 1] & 0xff) == 0xf7 ? message.length - 1 : message.length;
        if (end <= 3) {
            return message;
        }
    ...
    private static boolean looksLikeUnsupportedRawYamahaPacket(byte[] message) {
        if (message.length < 3) {
            return false;
        }
        int manufacturer = message[0] & 0xff;
        int family = message[1] & 0xff;
        return manufacturer == 0x43 && family <= 0x05;
    }

    private void debugUnsupportedRawYamahaPacket(byte[] message) {
        if (!debugControlEnabled()) {
            return;
        }
        int prefixLength = Math.min(message.length, 8);
        StringBuilder signature = new StringBuilder(prefixLength * 2);
        for (int i = 0; i < prefixLength; i++) {
            signature.append(String.format("%02x", message[i] & 0xff));
        }
        if (!loggedUnsupportedRawSysEx.add(signature.toString())) {
            return;
        }
        StringBuilder sb = new StringBuilder(128);
        sb.append("[FueTrekCtrl] unsupported-yamaha len=").append(message.length).append(" data=");
        for (int i = 0; i < message.length; i++) {
            if (i > 0) {
                sb.append(' ');
            }
    ...
    private static void debugPitchEvent(String source, ChannelState state, int rawParam) {
        if (!debugControlEnabled()) {
            return;
        }
        SmafDebug.debug("smaf", String.format(
                "[FueTrekPitch] %s ch=%d raw=0x%02x rawWord=0x%04x hi=0x%02x lo=0x%02x bendRange=%d",
                source,
                state.channelIndex,
                rawParam & 0xff,
                state.rawPitchWord,
    ...
    public void handleRawExtBEvent(int eventId, int channel, int rawParam) {
        if ((eventId & 0xff) == EVENT_X_DRUM_ENABLE) {
        ...
        switch (eventId & 0xff) {
            ...
            case EVENT_PITCHBEND:
                // FF4's live traces send centered 7-bit values here (for example
                // `0x42` / `0x3d` around the neutral `0x40` midpoint), so
                // treating `0xe4` as a split high-6 lane flips small bends into
                // near-full-range detunes. Keep the legacy low-lane path on
                // `0xe8/0xe9`, but decode `0xe4` itself as a complete centered
                // pitch word.
                state.setRawPitchWord(rawPitchWordFromCentered7(rawParam & 0x7f));
                debugPitchEvent("e4", state, rawParam);
                commitChannelPitch(state);
                return;
            case 0xe6:
            ...
            case EVENT_WAVE_CHANNEL_VOLUME:
                ...
                debugPitchEvent("e8", state, rawParam);
                commitChannelPitch(state);
                return;
            case EVENT_WAVE_CHANNEL_PANPOT:
                ...
                state.rawPitchLow6 = value;
                debugPitchEvent("e9", state, rawParam);
                return;
            case 0xea:
    ...
    private static int rawPitchWordFromCentered7(int value) {
        int centered = clampRange(value, 0, 0x7f) - 0x40;
        return clampRange(0x2000 + (centered << 7), 0, 0x3fff);
    }
    ...
    private ResolvedZone resolveZone(ChannelState state, SelectorState selector) {
        ...
        if (state.rawGroupOverride >= 0 && state.rawObjectOverride >= 0) {
            ResolvedZone rawOverride = resolveObjectZone(state.rawGroupOverride, state.rawObjectOverride, selector.noteByte);
            if (rawOverride != null) {
                return rawOverride;
            }
        }
    ...
    private ResolvedZone resolveObjectZone(int groupId, int objectIndex, int noteByte) {
        ...
        FueTrekRom.ObjectHeader object = group.entry(objectIndex);
        if (object == null || noteByte < object.lowKey) {
            return null;
        }
        for (FueTrekRom.Zone zone : object.zones) {
            if (zone != null && noteByte <= zone.keyHigh) {
                return new ResolvedZone(zone, groupId, 0, objectIndex, noteByte);
            }
        }
        return null;
    }

    private int allocateVoiceSlot(Voice incoming) {
        for (int i = 0; i < voiceSlots.length; i++) {
        ...
        for (int i = 0; i < voiceSlots.length; i++) {
            ...
            if (bestIndex < 0 || shouldStealVoice(voice, voiceSlots[bestIndex], incoming)) {
                bestIndex = i;
            }
        }
        return bestIndex;
    ...
    private static boolean shouldStealVoice(Voice candidate, Voice currentBest, Voice incoming) {
        if (currentBest == null) {
            return true;
        }
        int candidateRelease = candidate.releasePriority();
        int bestRelease = currentBest.releasePriority();
        if (candidateRelease != bestRelease) {
            return candidateRelease > bestRelease;
        }
        int candidateLevel = candidate.currentLevelWord();
        int bestLevel = currentBest.currentLevelWord();
        if (candidateLevel != bestLevel) {
            return candidateLevel < bestLevel;
        }
        int candidateNotePriority = candidate.notePriority();
        int bestNotePriority = currentBest.notePriority();
        if (candidateNotePriority != bestNotePriority) {
            return candidateNotePriority < bestNotePriority;
        }
        boolean candidateMatchesIncoming = incoming != null
                && candidate.channelIndex == incoming.channelIndex
                && candidate.key == incoming.key;
        boolean bestMatchesIncoming = incoming != null
                && currentBest.channelIndex == incoming.channelIndex
                && currentBest.key == incoming.key;
        if (candidateMatchesIncoming != bestMatchesIncoming) {
            return candidateMatchesIncoming;
        }
    ...
    private static final class LegacyYamahaSlotState {
        ...
        int selectorProgramByte;
        int selectorBankByte;
        int phraseVolumeByte;
        int volumeByte;
        int expressionByte;
        int panByte;
        int modulationByte;
        ...
        void reset() {
            ...
            selectorProgramByte = 0;
            selectorBankByte = 0;
            phraseVolumeByte = 0x64;
            volumeByte = 0x64;
            expressionByte = 0x64;
            panByte = 0x40;
            modulationByte = 0;
        ...
        boolean matchesActiveNote(int noteWord, int key) {
            return activeNoteWord == noteWord;
        }
        ...
        float channelVolume() {
            return combineVolumeByteLanes(phraseVolumeByte, volumeByte) / 127.0f;
        }

        float velocity() {
            return combineVolumeByteLanes(combineVolumeByteLanes(phraseVolumeByte, volumeByte), expressionByte) / 127.0f;
        }
    ...
    private static final class DecodedCompactValue {
        final int value;
        final int nextIndex;

        DecodedCompactValue(int value, int nextIndex) {
            this.value = value;
            this.nextIndex = nextIndex;
        }
    }

    private static final class RawExvoVoicePoint {
        final int modeA;
        final int modeB;
        final int x;
        final int y;

        RawExvoVoicePoint(int modeA, int modeB, int x, int y) {
            this.modeA = modeA;
            this.modeB = modeB;
            this.x = x;
            this.y = y;
    ...
    private static final class RawExvoProgramSlot {
        ...
        final int legacyBankByte;
        final int legacyProgramByte;
        ...
        RawExvoProgramSlot(
                ...
                int legacyBankByte,
                int legacyProgramByte) {
            this.programSlot = programSlot & 0x3f;
            this.groupId = groupId;
            this.objectIndex = objectIndex;
            this.voiceProgram = voiceProgram;
            this.upload81 = upload81;
            this.legacyBankByte = legacyBankByte;
            this.legacyProgramByte = legacyProgramByte;
        }
    ...
    private static final class RawExvoUpload81 {
        ...
        String primaryPairHex() {
            ...
            return String.format("%02x%02x", payload[0] & 0xff, payload[1] & 0xff);
        }

        String secondaryPairHex() {
            if (payload.length < 4) {
                return "--";
            }
    ...
    private static final class ChannelState {
        ...
        int rawGroupOverride = -1;
        int rawObjectOverride = -1;
        final TemplateState template = new TemplateState();
        int linkSource = -1;
        boolean auxPairLoaded = false;
        int auxPitchWord = 0x400;
        int auxNote = 0x15;
        RawExvoVoiceProgram rawVoiceProgram;
        RawExvoUpload81 rawUpload81;
        ...
        void clearLegacyYamahaBaseObject() {
            selectedZoneOverride = null;
            rawGroupOverride = -1;
            rawObjectOverride = -1;
        }
        ...
        boolean applyRawControlTemplate(FueTrekRom rom, int groupId, int objectIndex) {
            FueTrekRom.Group group = rom.group(groupId);
            if (group == null) {
                return false;
            }
            ...
            for (FueTrekRom.Zone zone : object.zones) {
                ...
                // Raw `43 05 02 01 ...` packets can act as the live EXVO source
                // when no follow-up `43 05 01 01 ...` voice packet arrives on the
                // channel. Keep the zone selected until a later raw voice override
                // supersedes it, while still exposing the control/template fields
                // to the note path.
                selectedZoneOverride = new ResolvedZone(zone, groupId, 0, objectIndex, object.lowKey);
                template.loadFromZone(zone);
                rawGroupOverride = groupId & 0xff;
                rawObjectOverride = objectIndex & 0x7f;
                return true;
        ...
        private void applyLinkControl(ChannelState[] channels, byte[] payload, int offset) {
            ...
            linkSource = source;
            auxPairLoaded = leader.auxPairLoaded;
        ...
        private void propagateAuxPair(ChannelState[] channels, int sourceIndex) {
            for (int i = 0; i < channels.length; i++) {
                if (i != sourceIndex && channels[i].linkSource == sourceIndex) {
                    channels[i].auxPairLoaded = auxPairLoaded;
    ...
    private static final class Voice {
        ...
        Voice(
            ...
            this.staticPitchQ16 = computeStaticPitchQ16(channel, resolved.noteByte, baseNote, template);
        ...
        int currentLevelWord() {
            int envLevel = Math.abs(envA.output) + Math.abs(envB.output);
            int mixLevel = mixLeftWord + mixRightWord;
            return envLevel + mixLevel;
        }

        int notePriority() {
            return FueTrekNoteShapeTables.noteShapeIndex(midiKey);
        }
        ...
        private static int computeStaticPitchQ16(
                ChannelState channel, int midiKey, int baseNote, VoiceTemplate template) {
            if (!channel.auxPairLoaded || template.notePitchMulWord == 0) {
                return 0;
            }
            // Native large-control pitch pairs provide an anchor note plus a Q5
            // root-word bias. `notePitchMulWord` then scales note motion away from
            // that anchor, so convert the resulting Q5 note word back into the
            // Q16 semitone domain used by `updatePitchWords`.
            int anchorBiasQ5 = channel.auxPitchWord - 0x400;
            int anchorDelta = midiKey - channel.auxNote;
            int anchoredWordQ5 = (channel.auxNote << 5) + anchorBiasQ5 + (anchorDelta * template.notePitchMulWord);
            int baselineWordQ5 = baseNote << 5;
            return (anchoredWordQ5 - baselineWordQ5) << 11;
        }
