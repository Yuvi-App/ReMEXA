...
public final class M3GLoader {
	...
	private static final Boolean NOT_REFERENCED = Boolean.FALSE;
	private static final Boolean REFERENCED = Boolean.TRUE;
		...
		if (this.currentSection > 1 && this.externalLinks && !this.containedExternalLinks) {
			...
			if ((compression = readByte(ais = new AdlerInputStream(var1))) == -1) {
				...
				if (compression == 0) {
					...
					if (compression != 1) {
						System.err.println("M3GLoader: unrecognized compression scheme (" + compression + ").");
						return false;
					}
	...
	private void readObject3DData(Object3D var1, InputStream var2) throws IOException {
		...
		if (var6 != 0) {
			...
			while (var6-- > 0L) {
				...
				var2.read(var9);
				var10.put(Integer.valueOf(var8), var9);
			}
	...
	private static int getInnerFileType(byte[] file, int offset) {
		...
		if (missmatchedBytes == 0) {
			...
			if (missmatchedBytes == 0) {
				...
				System.err.println("M3GLoader: invalid file type, assuming PNG.");
				return 2;
			}
		}
	}
	...
	private InputStream getInputStream(String var1) throws IOException {
		if (hasScheme(var1)) {
			return isRemoteUri(var1) ? getHttpInputStream(var1) : openAppResource(var1);
		} else if (var1.charAt(0) == 47) {
			return openAppResource(var1);
		} else if (this.parentResourceName == null) {
			throw new IOException("Relative URI.");
		} else {
			String var2 = this.parentResourceName.substring(0, this.parentResourceName.lastIndexOf(47) + 1) + var1;
			return isRemoteUri(var2) ? getHttpInputStream(var2) : openAppResource(var2);
		}
	}
	...
	private static void inflate(byte[] var0, byte[] var1) {
		try {
			...
			System.err.println("M3GLoader: unzip error.");
		}
	}
	...
	private static boolean isRemoteUri(String location) {
		return location.regionMatches(true, 0, "http:", 0, 5)
				|| location.regionMatches(true, 0, "https:", 0, 6);
	}

	private static InputStream openAppResource(String location) throws IOException {
		InputStream stream = MidletRuntime.openResource(location);
		if (stream == null) {
